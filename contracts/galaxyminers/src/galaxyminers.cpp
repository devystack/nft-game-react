...
void nicegm::settmconf(std::string name, std::uint32_t template_id, std::string type, std::string img, std::string badge_img, std::uint8_t saved_claims, std::uint8_t additional_tools, std::uint16_t additional_energy, std::uint8_t shard_production, std::uint8_t rewards_rate, std::uint8_t lucky, std::vector<eosio::asset> mints, std::uint64_t unstaking_time, std::uint64_t charged_time) {
  
  require_auth(get_self());

  tmconfig item(_self, _self.value);
  ...
  item.emplace(_self, [&](auto &i)
                      {
                        ...
                        i.additional_tools = additional_tools;
                        i.additional_energy = additional_energy;
                        i.shard_production = shard_production;
...
void nicegm::deltmconf(std::uint32_t template_id) {
...
void nicegm::settmcraft(std:: uint32_t id, std::string name, std::string img, std::uint8_t saved_claims, std::uint8_t additional_tools, std::uint16_t additional_energy, std::uint8_t shard_production, std::uint8_t rewards_rate, std::uint8_t lucky, std::vector<eosio::asset> mints, std::uint64_t unstaking_time, std::uint64_t charged_time) {

  require_auth(get_self());
  tmcraft item(_self, _self.value);

  auto current_item = item.find(id);
...
//                (addplayer)(addconfitem)(delconfitem)(addsetting)(delsetting)(settmconf)(deltmconf)(settmcraft)(tassetmint)(withdraw)(transfers)(addgadget)(detachgadget)(addconftool)(claim)(repair)(recover)(logopenpack)(addconfmkt)(delconfmkt)(managewl)(testclaim)(testenergy) // admin management
// );
